...
//	filename = unescape(filename);
	...
	if (option==0) {	// rename
		...
		titleObj.innerText = "重命名";
		titleObj.textContent = "重命名";
		document.getElementById("modal-title").appendChild(titleObj);
		
		var textObj = document.createElement("a");
		textObj.innerText = "当前文件名 "+"\""+filename+"\"";
		textObj.textContent = "当前文件名 "+"\""+filename+"\"";
		...
		document.getElementsByName('submitButton')[0].value = "重命名";
		document.getElementsByName('ID')[0].value = filename;
	}
	else if (option==1) { // create folder
		var titleObj = document.createElement("h3");
		titleObj.innerText = "新建目录";
		titleObj.textContent = "新建目录";
		document.getElementById("modal-title").appendChild(titleObj);
		
		var textObj = document.createElement("a");
		textObj.innerText = "请输入新建目录名";
		textObj.textContent = "请输入新建目录名";
		document.getElementById("modal-text").appendChild(textObj);
        
		var fieldObj = document.createElement("input");
		fieldObj.setAttribute("type","input");
		fieldObj.setAttribute("name","targetItem");
		fieldObj.setAttribute("value","新建目录");
		...
		// assign text field value
		document.getElementsByName('submitButton')[0].value = "创建";	
		...
		var textObj = document.createElement("a");
		textObj.innerText = "您确定删除这个文件吗？\n" + "\""+filename+"\"";
		textObj.textContent = "您确定删除这个文件吗？\n" + "\""+filename+"\"";
		document.getElementById("modal-text").appendChild(textObj);
		...
		// assign text field value
		document.getElementsByName('submitButton')[0].value = "删除";	
		document.getElementsByName('ID')[0].value = filename;
	}
    
	// pop modal
	$('#modal-content').modal({closeHTML: "<input type=button name='cancelButton' id='cancelButton' value='取消'>"});
}
...
function deleteFile(url, params) {
	var answer = confirm("您确定要删除这个文件吗？\n" + params['deleteFile'])
...
qq.FileUploader = function(o){
    ...
    qq.extend(this._options, {
        ...
        template: '<div class="qq-uploader">' + 
                '<div class="qq-upload-drop-area"><span>拖拽文件到这开始上传</span></div>' +
                '<div class="qq-upload-button"><img src="/root_989234ylsdfyy87e/icon_up.png" width="16" height="16"/>  上传文件</div>' +
        ...
        fileTemplate: '<li>' +
				...
				'<span class="qq-upload-exist-text">File Exist</span>' +
            '</li>',        
...
qq.extend(qq.FileUploader.prototype, {
    ...
    _setupDragDrop: function(){
        ...
        var dz = new qq.UploadDropZone({
            ...
            onDrop: function(e){
				// add to hide drag & drop are on IE 
				if(qq.UploadHandlerXhr.isSupported()){           
